...
export default function DiscoverClient({ users, userPp, modePp, friendIds, allRooms, lobbyExtras, canSendDm, defaultStarRange }: Props) {
  ...
  const [lobbyNotice, setLobbyNotice] = useState<{ type: 'warn' | 'error'; text: string } | null>(null);
    ...
    <>
      <div className="mb-4">
        <h2 className="text-lg font-semibold flex items-center gap-2 mb-0.5">
          ...
          {lobbyNotice && (
            <div className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg text-xs whitespace-nowrap ${
              lobbyNotice.type === 'error'
              ...
              {lobbyNotice.type === 'error' ? (
                ...
                <svg className="w-3 h-3 shrink-0 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
