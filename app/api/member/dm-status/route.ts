...
export async function GET(req: NextRequest) {
  ...
  if (!token) return NextResponse.json({ onCooldown: false });

  const targetId = Number(req.nextUrl.searchParams.get('targetId'));
  if (!targetId) return NextResponse.json({ onCooldown: false });

  const recent = await prisma.memberDm.findFirst({
    where: {
  ...
  if (!recent) return NextResponse.json({ onCooldown: false });
