...
export async function fetchUserProfile(osuId: number, mode = 'osu', revalidate?: number) {
  ...
  return {
    ...
    playmode: (u.playmode ?? 'osu') as string,
    ...
    joinDate: u.join_date ? new Date(u.join_date as string) : null,
    team: u.team
      ? {
          id: String(u.team.id),
...
export async function fetchUserScoreOnBeatmap(osuUserId: number, beatmapId: string, mode = 'osu'): Promise<{ pp: number } | null> {
  const token = await getClientToken();
  const res = await fetch(
    `https://osu.ppy.sh/api/v2/beatmaps/${beatmapId}/scores/users/${osuUserId}?ruleset=${mode}`,
